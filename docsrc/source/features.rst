...
- | A RSS feed is available in the user profile with the 5 last public workouts (displayed by default in English and using the metric system) (*new in 0.12.0*).
  ...
  | Workout description can be displayed by adding `description` parameter (not displayed by default) (*new in x.x.x*):

  - ``https://<FITTRACKEE_DOMAIN>/users/<USERNAME>/workouts.rss?description=tree``

  | The rendering of the description may differ from that on FitTrackee, as additional styles are applied on the UI.
