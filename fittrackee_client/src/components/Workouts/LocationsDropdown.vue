    ...
    >
      ...
      >
        {{ location.display_name }} ({{
          t(`workouts.NOMATIM_ADDRESS_TYPE.${location.addresstype}`)
        }})
      </li>
    </ul>
  </div>
...
<script setup lang="ts">
  ...
  function onUpdateLocation(index: number) {
    ...
    if (locations.value.length > index) {
      locationDisplayName.value = `${locations.value[index].display_name} (${t(`workouts.NOMATIM_ADDRESS_TYPE.${locations.value[index].addresstype}`)})`
      ...
      if (!newValue) {
        ...
        locations.value.filter(
          ({ display_name, addresstype }) =>
            `${display_name} (${t(`workouts.NOMATIM_ADDRESS_TYPE.${addresstype}`)})` ===
            newValue
        ).length === 0
      ) {
