...
from .nomatim_service import NomatimService
...
def get_coordinates_from_city(
    ...
    """
    Return coordinates based on location (city) using Nomatim API.
    ...
    **Example responses**:
    ...
    if city:
        try:
            locations = nomatim_service.get_locations_from_city(city)
        except Exception:
...
def get_location_from_id(auth_user: "User") -> Union[Dict, "HttpResponse"]:
    """
    Return location based on OSM id using Nomatim API.
    ...
    **Example responses**:
    ...
    if osm_id:
        try:
            location = nomatim_service.get_location_from_id(osm_id)
        except Exception:
