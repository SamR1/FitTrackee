...
from fittrackee.geocode.nomatim_service import NomatimService
...
class TestNomatimServiceInstantiation:
    def test_it_instantiates_service_when_no_nomatim_url_set_in_env_var(
        self, app: "Flask"
    ) -> None:
    ...
    def test_it_instantiates_service_when_nomatim_url_is_set_in_env_var(
        self, app_with_nomatim_url: "Flask"
    ) -> None:
        service = NomatimService()
...
class TestNomatimServiceGetLocationFromId(ResponseMockMixin):
    def test_it_calls_nomatim_api_with_given_oms_id(
        self, app: "Flask"
    ) -> None:
        service = NomatimService()
        osm_id = "r71525"
    ...
    def test_it_returns_location_data(
        ...
        with patch.object(
            requests,
            "get",
            return_value=self.get_response([nomatim_response[0]]),
        ):
