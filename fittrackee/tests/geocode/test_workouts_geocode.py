...
class TestGeocodeGetCoordinatesFromLocation(GeocodeTestCase):
    ...
    def test_it_returns_empty_list_when_no_query_provided(
        ...
        with patch(
            "fittrackee.geocode.routes.nomatim_service.get_locations_from_query",
            return_value=[],
        ) as nomatim_service_mock:
            response = client.get(
                self.route,
                headers=dict(Authorization=f"Bearer {auth_token}"),
            )

        nomatim_service_mock.assert_not_called()
        assert response.status_code == 200
        data = json.loads(response.data.decode())
        assert data["status"] == "success"
        assert data["locations"] == []

    def test_it_returns_500_when_nomatim_api_returns_error(
        self,
        app: "Flask",
        user_1: "User",
        sport_1_cycling: "Sport",
    ...
    def test_it_returns_nomatim_api_response(
        self,
        app: "Flask",
        user_1: "User",
    ) -> None:
        query = "Paris"
        nomatim_service_response = [
            {
                "addresstype": "suburb",
        ...
        with patch(
            "fittrackee.geocode.routes.nomatim_service.get_locations_from_query",
            return_value=nomatim_service_response,
        ) as nomatim_service_mock:
            response = client.get(
        ...
        nomatim_service_mock.assert_called_once_with(query)
        assert response.status_code == 200
        data = json.loads(response.data.decode())
        assert data["status"] == "success"
        assert data["locations"] == nomatim_service_response
...
class TestGeocodeGetLocationFromId(GeocodeTestCase):
    ...
    def test_it_returns_empty_dict_when_no_osm_id_provided(
        ...
        with patch(
            "fittrackee.geocode.routes.nomatim_service.get_location_from_id",
            return_value=[],
        ) as nomatim_service_mock:
            response = client.get(
                self.route,
    ...
    def test_it_returns_nomatim_api_response(
        ...
        with patch(
            "fittrackee.geocode.routes.nomatim_service.get_location_from_id",
            return_value=nomatim_service_response[0],
        ) as nomatim_service_mock:
            response = client.get(
        ...
        nomatim_service_mock.assert_called_once_with(osm_id)
        assert response.status_code == 200
        data = json.loads(response.data.decode())
        assert data["status"] == "success"
        assert data["location"] == nomatim_service_response[0]
