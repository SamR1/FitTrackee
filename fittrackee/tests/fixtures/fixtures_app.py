...
def app(monkeypatch: pytest.MonkeyPatch) -> Generator:
    ...
    if os.getenv("NOMATIM_URL"):
        monkeypatch.delenv("NOMATIM_URL")
...
@pytest.fixture
def app_with_nomatim_url(monkeypatch: pytest.MonkeyPatch) -> Generator:
    monkeypatch.setenv("NOMATIM_URL", "https://nominatim.example.com")
    yield from get_app(with_config=True)
